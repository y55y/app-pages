...
export const PRINTABLE_PARAM_MAPPERS = {
  ...
  daily_entry: {
    journal_id: (data) => data.Number || data.qidID || data.ID || data.id || '',
    journal_date: (data) => formatDate(data.qidDate || data.Date || data.CreatedDate),
    journal_reference: (data) => data.qidRef || data.Ref || data.Reference || '',
    journal_description: (data) => data.qidnote || data.description || data.Description || data.Note || '',
    journal_currency_code: (data) => data.CurrencyCode || data.currency_code || data.currency || '',
    journal_currency_rate: (data) => data.CurrencyRate || data.currency_rate || data.rate || '',
    journal_creation_date: (data) => formatDate(data.CreatedDate || data.created_at),
    ...
    items: (data) => {
      ...
      const rows = parseRows(data.JournalTransaction)
        .concat(parseRows(data.journal_transactions_table))
        .concat(parseRows(data.entries))
        .concat(parseRows(data.rows))
        .concat(parseRows(data.qidE))
        .concat(parseRows(data.qid));
      const sourceRows = rows.length ? rows : (data.qidEAmountD !== undefined || data.qidEAmountC !== undefined ? [data] : []);
      return sourceRows.map((row) => {
        ...
        return {
          ...
          description: row.description || row.qidEnote || row.note || '',
    ...
    journal_total_debit: (data) => {
      const rows = PRINTABLE_PARAM_MAPPERS.daily_entry.items(data);
      const total = data.TotalDebit ?? data.total_debit ?? data.journal_total_debit ?? rows.reduce((sum, row) => sum + Number(String(row.debit || 0).replace(/,/g, '')), 0);
      return formatAmount(total);
    },
    journal_total_credit: (data) => {
      const rows = PRINTABLE_PARAM_MAPPERS.daily_entry.items(data);
      const total = data.TotalCredit ?? data.total_credit ?? data.journal_total_credit ?? rows.reduce((sum, row) => sum + Number(String(row.credit || 0).replace(/,/g, '')), 0);
      return formatAmount(total);
    },
    journal_total_debit_without_symbol: (data) => formatAmount(data.TotalDebit ?? data.total_debit ?? 0),
    journal_total_credit_without_symbol: (data) => formatAmount(data.TotalCredit ?? data.total_credit ?? 0),
    cost_center_table: () => ''
  },
