...
export const PRINTABLE_PARAM_MAPPERS = {
  ...
  sell_inv: {
    ...
    items: (data) => {
      ...
      if (!Array.isArray(rawItems)) return [];
       ...
       return rawItems.map((item, index) => {
        ...
        const rawTaxValue = item.TaxA ?? item.tax_value ?? item.taxValue;
         const taxableShare = taxableTotals > 0 ? taxableAmount / taxableTotals : 0;
         const taxValue = hasHeaderVat
           ? Math.round(headerVat * taxableShare * 100) / 100
           : (rawTaxValue === undefined || rawTaxValue === null || rawTaxValue === '' || !Number.isFinite(Number(rawTaxValue))
             ? calculatedTax
             : Number(rawTaxValue));
    ...
    total_tax: (data) => {
      ...
      const rowVat = entries.reduce((sum, item) => {
        ...
        const rawTax = item.TaxA ?? item.tax_value ?? item.taxValue;
        return sum + (rawTax === undefined || rawTax === null || rawTax === '' || !Number.isFinite(Number(rawTax))
          ? taxable * rate / 100 : Number(rawTax));
      }, 0);
      return formatAmount(headerVat > 0 ? headerVat : rowVat);
    },
    total_amount: (data) => {
      const d = data.data || data;
      const entries = d.order_entries || d.Items || [];
      ...
      if (headerTotal > 0) return formatAmount(headerTotal);
      if (!entries.length) return formatAmount(Number(d.vat ?? d.VAT ?? d.Tax ?? 0));
      const total = entries.reduce((sum, item) => {
        ...
        const rate = rawRate === undefined || rawRate === null || rawRate === '' || !Number.isFinite(Number(rawRate)) ? 15 : Number(rawRate);
        const rawTax = item.TaxA ?? item.tax_value ?? item.taxValue;
        const tax = rawTax === undefined || rawTax === null || rawTax === '' || !Number.isFinite(Number(rawTax)) ? taxable * rate / 100 : Number(rawTax);
        return sum + taxable + tax;
      }, 0);
      return formatAmount(total - discount);
    },
  ...
  // Sell Quote Draft - first-year amount plus non-billing renewal details
  sell_quote_draft: {
    __inherit: 'sell_inv',
    next_years_renewal: (data) => formatAmount((data.data || data).NextYearsRenewal || (data.data || data).next_years_renewal || 0)
  },
  ...
  buy_inv: {
    ...
    subtotal: (data) => {
      ...
      if (Array.isArray(items) && items.length) {
        const formatted = formatInvoiceItems(items);
        return formatAmount(formatted.reduce((sum, item) => sum + item.taxable_amount, 0));
      }
      let value = 0;
      if (d.SubTotal || d.Subtotal) {
    ...
    total_tax: (data) => {
      ...
      if (Array.isArray(items) && items.length) {
        const formatted = formatInvoiceItems(items);
        return formatAmount(formatted.reduce((sum, item) => sum + item.tax_value, 0));
      }
      let value = 0;
    ...
    total_amount: (data) => {
      ...
      if (Array.isArray(items) && items.length) {
        const formatted = formatInvoiceItems(items);
        return formatAmount(formatted.reduce((sum, item) => sum + item.total, 0));
      }
      let value = 0;
      if (d.Total) {
        value = d.Total || 0;
    ...
    total_without_vat: (data) => {
      ...
      return PRINTABLE_PARAM_MAPPERS.sell_inv.total_without_vat(data);
    },
    footer: (data, user) => {
      const d = data.data || data;
