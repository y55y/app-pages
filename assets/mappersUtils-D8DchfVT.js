...
export function calculateTotals(data) {
  ...
  const entries = data.order_entries || data.Items || data.items || [];
  ...
  if (sub === 0 && entries.length) {
    sub = entries.reduce((sum, item) => {
      ...
      const storedTotal = item.Total ?? item.total;
      return sum + (storedTotal !== undefined && storedTotal !== null && storedTotal !== ''
  ...
  if (discount === 0 && entries.length) {
    discount = entries.reduce((sum, item) => {
  ...
  if (vat === 0 && entries.length) {
    vat = entries.reduce((sum, item) => {
      const rawVat = item.TaxA ?? item.tax_value ?? item.taxValue;
      if (rawVat !== undefined && rawVat !== null && rawVat !== '') return sum + Number(rawVat || 0);
      const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
      const price = Number(item.unitPrice ?? item.Rate ?? item.Price ?? item.price ?? 0);
      const lineDiscount = Number(item.discount || item.dis || 0);
      const taxRate = Number(item.tax_rate ?? item.TaxRateSnapshot ?? item.TaxRate ?? 15);
      return sum + ((qty * price - lineDiscount) * taxRate / 100);
    }, 0);
  }
