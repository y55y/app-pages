...
const formatSessionAmount = (data, value) => {
  const currency = data?.currency_code || data?.currency?.code || 'SAR';
  const decimals = new Intl.NumberFormat('en-US', { style: 'currency', currency }).resolvedOptions().maximumFractionDigits;
  return formatAmount(value, decimals);
};

export const PRINTABLE_PARAM_MAPPERS = {
  pos_session_report: {
    CashairSunit: (data) => String(data?.CashairSunit ?? data?.invoice_count ?? ''),
    CashairSunitR: (data) => String(data?.returnInvoiceCount ?? data?.CashairSunitR ?? 0),
    CashairSID: (data) => data?.CashairSID || '',
    CashairSstore: (data) => data?.branch_name || data?.branch_name_en || '',
    CashairScID: (data) => [data?.CashierID, data?.CashierName].filter(Boolean).join(' — '),
    CashairSU: (data) => [data?.UserID, data?.UserName].filter(Boolean).join(' — '),
    session_status: (data) => ({ closed: 'مغلقة / Closed', pending_review: 'قيد المراجعة / Pending Review', open: 'مفتوحة / Open' }[String(data?.session_status || '').toLowerCase()] || data?.session_status || ''),
    CashairSstart: (data) => formatSessionDate(data?.CashairSstart),
    CashairSend: (data) => formatSessionDate(data?.CashairSend),
    CashairStotal: (data) => formatSessionAmount(data, data?.CashairStotal),
    CashairDIS: (data) => formatSessionAmount(data, data?.CashairDIS),
    CashairRChange: (data) => formatSessionAmount(data, data?.CashairRChange),
    CashairSubTotal: (data) => formatSessionAmount(data, data?.CashairSubTotal),
    CashairPostpond: (data) => formatSessionAmount(data, data?.CashairPostpond),
    CashairSopen: (data) => formatSessionAmount(data, data?.CashairSopen),
    CashairPAID: (data) => formatSessionAmount(data, data?.CashairPAID),
    CashairRTN: (data) => formatSessionAmount(data, data?.CashairRTN),
    CashairRCASH: (data) => formatSessionAmount(data, data?.CashairRCASH),
    totalCashPayments: (data) => formatSessionAmount(data, Number(data?.CashairRTN || 0) + Number(data?.CashairRCASH || 0)),
    CashairRFUN: (data) => formatSessionAmount(data, data?.CashairRFUN),
    CashairNet: (data) => formatSessionAmount(data, data?.CashairNet),
    CashairSclose: (data) => formatSessionAmount(data, data?.CashairSclose),
    CashairCARD: (data) => formatSessionAmount(data, data?.CashairCARD),
    CashairCARDt: (data) => formatSessionAmount(data, data?.CashairCARDt),
    OtherPayment: (data) => formatSessionAmount(data, data?.OtherPayment),
    paymentMethodDifference: (data) => formatSessionAmount(data, data?.paymentMethodDifference),
    paymentMethodDifferenceClass: (data) => signClass(data?.paymentMethodDifference),
    CashairCARDdiff: (data) => formatSessionAmount(data, data?.CashairCARDdiff),
    CashairCARDdiffClass: (data) => signClass(data?.CashairCARDdiff),
    cashDifference: (data) => formatSessionAmount(data, data?.cashDifference),
    cashDifferenceClass: (data) => signClass(data?.cashDifference),
    CashairSHORT: (data) => formatSessionAmount(data, data?.CashairSHORT),
    CashairSHORTClass: (data) => signClass(data?.CashairSHORT),
    category_sales_html: (data) => (Array.isArray(data?.category_sales) ? data.category_sales : []).map((row) => \`<tr><td class="category-name">\${String(row.name || '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]))}</td><td class="num">\${formatSessionAmount(data, row.amount)}</td><td class="num">\${formatAmount(row.percentage, 2)}%</td></tr>\`).join(''),
    payment_methods_html: (data) => {
      const raw = data?.payment_methods ?? data?.PaymentMethods ?? data?.PaymentMethodBreakdown ?? data?.payments ?? data?.Payments ?? data?.reported_payment_methods ?? data?.ReportedPaymentMethods ?? [];
      let source = raw;
      if (typeof source === 'string') { try { source = JSON.parse(source); } catch { source = []; } }
      if (!Array.isArray(source) && source && typeof source === 'object') source = Object.entries(source).map(([name, value]) => ({ name, actual: value }));
      if (!Array.isArray(source)) source = [];
      const esc = (value) => String(value).replace(/[&<>\\"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '\\"': '&quot;', "'": '&#39;' }[char]));
      const rows = source.map((row) => {
        const name = row?.name ?? row?.Name ?? row?.payment_method_name ?? row?.PaymentMethodName ?? row?.method_name ?? row?.MethodName ?? row?.method ?? row?.Method ?? row?.PaymentMethod ?? row?.payment_method ?? row?.payment_method_id ?? '';
        const amount = row?.actual ?? row?.Actual ?? row?.actual_amount ?? row?.ActualAmount ?? row?.reported_amount ?? row?.ReportedAmount ?? row?.amount ?? row?.Amount ?? row?.total ?? row?.Total ?? row?.expected_amount ?? row?.ExpectedAmount ?? 0;
        return name ? \`<tr><td class="method">\${esc(name)}</td><td>\${esc(formatAmount(amount))}</td></tr>\` : '';
      }).filter(Boolean).join('');
      return rows ? \`<section class="methods-section"><div class="methods-wrap"><div class="methods-head"><div class="en">Other Payment Methods</div><div class="ar">وسائل الدفع الأخرى</div></div><table class="methods-table"><colgroup><col class="name"/><col class="sales"/><col class="actual"/><col class="diff"/></colgroup><thead><tr><th><span class="ar">وسيلة الدفع</span><span class="en">Method</span></th><th><span class="ar">المتوقع</span><span class="en">Expected</span></th><th><span class="ar">الفعلي</span><span class="en">Actual</span></th><th><span class="ar">الفرق</span><span class="en">Difference</span></th></tr></thead><tbody>\${source.map((row) => { const name = row?.name ?? row?.Name ?? row?.payment_method_name ?? row?.PaymentMethodName ?? row?.method_name ?? row?.MethodName ?? row?.method ?? row?.Method ?? row?.PaymentMethod ?? row?.payment_method ?? row?.payment_method_id ?? ''; const expected = Number(row?.expected_amount ?? row?.ExpectedAmount ?? 0); const actual = Number(row?.actual ?? row?.Actual ?? row?.reported_amount ?? row?.ReportedAmount ?? expected); const difference = actual - expected; return name ? \`<tr><td class="method-name">\${esc(name)}</td><td class="num">\${esc(formatAmount(expected))}</td><td class="num">\${esc(formatAmount(actual))}</td><td class="num \${difference > 0 ? 'is-positive' : difference < 0 ? 'is-negative' : 'is-zero'}">\${esc(formatAmount(difference))}</td></tr>\` : ''; }).filter(Boolean).join('')}</tbody></table></div></section>\` : '';
    },
    payment_methods: (data) => {
      ...
      const rows = source.map((row) => {
        const name = row?.name ?? row?.Name ?? row?.payment_method_name ?? row?.PaymentMethodName ?? row?.method_name ?? row?.MethodName ?? row?.method ?? row?.Method ?? row?.PaymentMethod ?? row?.payment_method ?? row?.payment_method_id ?? '';
        const amount = row?.actual ?? row?.Actual ?? row?.actual_amount ?? row?.ActualAmount ?? row?.reported_amount ?? row?.ReportedAmount ?? row?.amount ?? row?.Amount ?? row?.total ?? row?.Total ?? row?.expected_amount ?? row?.ExpectedAmount ?? 0;
        return { name: String(name), actual: formatAmount(amount) };
      }).filter((row) => row.name);
      return rows;
    },
  },
  ...
  sell_inv: {
    ...
    client_phone: (data) => {
      const d = data.data || data;
      return d.Client_PhoneNumber || d.ClientPhone || d.ClientMobile || d.Client_MobileNumber || d.PhoneNumber || d.MobileNumber || d.client_phone || '';
    },
    client_vat: (data) => {
      const d = data.data || data;
    ...
    items: (data) => {
      ...
      if (!Array.isArray(rawItems)) return [];
       ...
       const headerVat = Number(d.vat ?? d.VAT ?? d.Tax);
       const hasHeaderVat = Number.isFinite(headerVat);
       const taxableTotals = rawItems.reduce((sum, item) => {
       ...
       return rawItems.map((item, index) => {
        ...
        const rate = Number(item.unitPrice || item.Rate || item.Price || 0);
        const discount = Number(item.discount || item.dis || 0);
        const rawTaxRate = item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate;
        const taxRate = rawTaxRate === undefined || rawTaxRate === null || rawTaxRate === '' || !Number.isFinite(Number(rawTaxRate))
          ? 15
          : Number(rawTaxRate);
        
        const taxableAmount = Math.round((qty * rate - discount) * 100) / 100;
        const calculatedTax = Math.round((taxableAmount * taxRate / 100) * 100) / 100;
        const rawTaxValue = item.TaxA ?? item.tax_value ?? item.taxValue ?? item.TaxAmount;
        const hasExplicitLineTax = rawTaxValue !== undefined && rawTaxValue !== null && rawTaxValue !== '' && Number.isFinite(Number(rawTaxValue));
         const taxableShare = taxableTotals > 0 ? taxableAmount / taxableTotals : 0;
         const taxValue = taxRate === 0
           ? 0
           : (hasExplicitLineTax
             ? Number(rawTaxValue)
             : (hasHeaderVat
               ? Math.round(headerVat * taxableShare * 100) / 100
               : calculatedTax));
        const total = Math.round(taxableAmount * 100) / 100;
        const totalWithTax = Math.round((taxableAmount + taxValue) * 100) / 100;
        
        const code = item.ItemLookupCode || item.ItemID_ItemLookupCode || item.item_code || item.code || item.BarCodeNo || item.barcode || item.Barcode || item.ItemID || '';
        const description = item.name || item.ItemName || item.Name || item.description || '';
        const descriptionE = item.nameE || item.ItemNameE || item.NameE || item.descriptionE || '';
        return {
          serial: index + 1,
          no: index + 1,
          line_number: index + 1,
          ...
          description,
          descriptionE,
          short_description: item.ShortDescription || item.short_description || item.shortDescription || item.DescriptionShort || item.Description || '',
          short_descriptionE: item.ShortDescriptionE || item.short_descriptionE || item.shortDescriptionE || item.DescriptionShortE || item.DescriptionE || item.Description || '',
          quantity: qty,
          qty: qty,
    ...
    subtotal: (data) => {
      ...
      const sub = entries.reduce((sum, item) => {
        const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
         const price = Number(item.unitPrice ?? item.Rate ?? item.Price ?? item.price ?? 0);
        return sum + (qty * price);
      }, 0);
      return formatAmount(sub);
    },
    amount_before_discount_with_vat: (data) => {
      const d = data.data || data;
      const taxRate = Number(d.tax_rate || d.TaxRate || 15);
      let sub = Number(d.SubTotal || d.Subtotal || 0);
      if (sub === 0 && d.order_entries) {
        sub = d.order_entries.reduce((sum, item) => {
          const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
           const price = Number(item.unitPrice ?? item.Rate ?? item.Price ?? item.price ?? 0);
          return sum + (qty * price);
        }, 0);
      }
      return formatAmount(sub + (sub * taxRate / 100));
    },
    ...
    total_without_vat: (data) => {
      ...
      const vat = Number(d.vat ?? d.VAT ?? d.Tax ?? 0);
      // Purchase totals may expose SubTotal as VAT-inclusive. Prefer the
      // persisted total less persisted VAT for an unambiguous net amount.
      if (total > 0 && vat >= 0) {
      ...
      if (d.SubTotal || d.Subtotal) {
        sub = Number(d.SubTotal || d.Subtotal || 0);
         ...
         if (sub === 0 && Number(d.Total || 0) > 0) {
           sub = Number(d.Total || 0) - Number(d.vat ?? d.VAT ?? d.Tax ?? 0);
         }
       }
    ...
    total_tax: (data) => {
      const d = data.data || data;
      const entries = d.order_entries || d.Items || [];
      const headerVat = Number(d.vat ?? d.VAT ?? d.Tax ?? 0);
      const hasRowVatDetails = entries.some(item => (item.TaxA !== undefined || item.TaxAmount !== undefined || item.TaxRateSnapshot !== undefined || item.tax_value !== undefined));
      const rowVat = entries.reduce((sum, item) => {
        const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
        const price = Number(item.unitPrice || item.Rate || item.Price || 0);
        const discount = Number(item.discount || item.dis || 0);
        const rawRate = item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate;
        const rate = rawRate === undefined || rawRate === null || rawRate === '' || !Number.isFinite(Number(rawRate)) ? 15 : Number(rawRate);
        const taxable = qty * price - discount;
        const rawTax = item.TaxA ?? item.tax_value ?? item.taxValue ?? item.TaxAmount;
        return sum + (rate === 0 ? 0 : (rawTax === undefined || rawTax === null || rawTax === '' || !Number.isFinite(Number(rawTax))
          ? taxable * rate / 100 : Number(rawTax)));
      }, 0);
      return formatAmount(hasRowVatDetails ? rowVat : (headerVat > 0 ? headerVat : rowVat));
    },
    total_amount: (data) => {
      const d = data.data || data;
      const entries = d.order_entries || d.Items || [];
      const discount = Number(d.dis ?? d.discount ?? 0);
      const headerTotal = Number(d.Total || 0);
      const hasRowDetails = entries.some(item => (item.QuantityOnOrder || item.Qty || item.Quantity) && (item.unitPrice || item.Rate || item.Price));
      if (!entries.length) return formatAmount(headerTotal > 0 ? headerTotal : Number(d.vat ?? d.VAT ?? d.Tax ?? 0));
      const total = entries.reduce((sum, item) => {
        const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
        const price = Number(item.unitPrice || item.Rate || item.Price || 0);
        const lineDiscount = Number(item.discount || item.dis || 0);
        const taxable = qty * price - lineDiscount;
        const rawRate = item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate;
        const rate = rawRate === undefined || rawRate === null || rawRate === '' || !Number.isFinite(Number(rawRate)) ? 15 : Number(rawRate);
        const rawTax = item.TaxA ?? item.tax_value ?? item.taxValue ?? item.TaxAmount;
        const tax = rate === 0 ? 0 : (rawTax === undefined || rawTax === null || rawTax === '' || !Number.isFinite(Number(rawTax)) ? taxable * rate / 100 : Number(rawTax));
        return sum + taxable + tax;
      }, 0);
      const calculatedTotal = total - discount;
      return formatAmount(hasRowDetails ? calculatedTotal : (headerTotal > 0 ? headerTotal : calculatedTotal));
    },
    ...
    payment_type: (data) => {
      const d = data.data || data;
      const raw = d.isCredit ?? d.IsCredit;
      return raw === true || Number(raw) === 1 ? 'آجل / Deferred' : 'فوري / Immediate';
    },
    payment_kind: (data) => {
      const d = data.data || data;
      const raw = d.isCredit ?? d.IsCredit;
    ...
    salesman: (data, user) => { const d = data.data || data; return d.CreatorName || d.SalesmanName || d.SellerName || d.UserName || d.Salesman || d.salesman || user?.name || user?.username || ''; },
    store: (data) => { const d = data.data || data; return d.StoreName || d.WarehouseName || d.Store || d.Warehouse || d.store || ''; },
    branch: (data) => { const d = data.data || data; return d.BranchName || d.Branch || d.branch || ''; },
    ...
    company_phone: (data, user) => { const c = getCompanyInfo(user); return c.company_phone || c.phone || ''; },
    company_email: (data, user) => { const c = getCompanyInfo(user); return c.company_email || c.email || ''; },
    company_website: (data, user) => { const c = getCompanyInfo(user); return c.company_website || c.website || ''; },
    bank_name: (data, user) => { const c = getCompanyInfo(user); return c.bank_name || c.bankName || ''; },
    bank_name_ar: (data, user) => { const c = getCompanyInfo(user); return c.bank_name_ar || c.bankNameAr || c.bank_name || c.bankName || ''; },
    bank_iban: (data, user) => { const c = getCompanyInfo(user); return c.bank_iban || c.iban || ''; },
  ...
  sell_order_draft: {
    ...
    items: (data) => {
      ...
      return rawItems.map((item, index) => {
        ...
        const taxableAmount = Math.round((qty * price - discount) * 100) / 100;
        ...
        return {
          ...
          note: item.note || item.Note || item.details || item.Details || '',
          quantity: qty,
          unit_price: formatAmount(price),
          rate: formatAmount(price),
  ...
  buy_inv: {
    ...
    invoice_date: (data) => {
      const d = data.data || data;
      return formatDate(d.InvDate || d.order_date || d.CreatedDate || d.Date || d.date || d.created_at || data.InvDate || data.order_date || data.CreatedDate || data.Date || data.date || data.created_at);
    },
    created_time: (data) => {
      const d = data.data || data;
    ...
    client_cr: (data) => {
      const d = data.data || data;
      return d.client_cr || d.Client_CRN || d.ClientCRN || d.Supplier_crn || d.SupplierCRN || d.CRN || '';
    },
    client_building_number: (data) => {
      const d = data.data || data;
      return d.client_building_number || d.Client_BuildingNumber || d.ClientBuildingNumber || d.BuildingNumber || '';
    },
    client_street_name: (data) => {
      const d = data.data || data;
      return d.client_street_name || d.Client_StreetName || d.ClientStreetName || d.StreetName || '';
    },
    client_city: (data) => {
      const d = data.data || data;
      return d.client_city || d.Client_City || d.ClientCity || d.City || '';
    },
    client_postal_code: (data) => {
      const d = data.data || data;
      return d.client_postal_code || d.Client_PostalCode || d.ClientPostalCode || d.Zip || '';
    },
    ...
    items: (data) => {
      ...
      return formatted.map((item, index) => {
        ...
        return {
          ...
          amount: formatAmount(item.total_with_tax),
          total: formatAmount(item.total),
          total_with_tax: formatAmount(item.total_with_tax),
          total_with_vat: formatAmount(item.total_with_tax),
          total_price: formatAmount(item.total_with_tax)
        };
      });
    },
    subtotal: (data) => {
      ...
      if (d.vat || d.VAT || d.Tax) {
        ...
        if (Array.isArray(items)) {
          value = items.reduce((sum, item) => {
            const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
            const rate = Number(item.unitPrice || item.Rate || 0);
            const discount = Number(item.discount ?? item.Discount ?? item.dis ?? item.discountAmount ?? 0);
            const taxRate = Number(item.tax_rate ?? item.TaxRate ?? item.TaxA ?? item.taxPercent ?? d.tax_rate ?? d.TaxRate ?? 15);
            const taxableAmount = Math.round((qty * rate - discount) * 100) / 100;
            const taxValue = Math.round((taxableAmount * taxRate / 100) * 100) / 100;
            return sum + taxValue;
          }, 0);
  ...
  income_receipt: {
    ...
    note: (data) => {
      const comments = Array.isArray(data?.sandE)
        ? data.sandE.map((entry) => entry?.sand_comment).filter(Boolean)
        : [];
  ...
  exepnse_receipt: {
    ...
    expense_id: (data) => data.Number || data.number || data.id || data.ID || '',
    amount: (data) => formatAmount(data.amount ?? data.Amount ?? 0),
    date: (data) => formatDate(data.date || data.CreatedDate),
    account_names: (data) => {
      const names = Array.isArray(data?.sandE)
        ? data.sandE.map((entry) => entry?.ACC_AccName || entry?.sand_ACCName || entry?.AccName || entry?.AccNames).filter(Boolean)
        : [];
      return names.join('، ') || data?.AccNames || data?.SandCat_Name || '';
    },
    client_business_name: (data) => getClientOrVendorName(data),
    vendor: () => '',
    client_bussiness_name_or_journal_name: (data) => getClientOrVendorName(data),
    spelled_amount: (data) => spellAmount(data.amount ?? data.Amount ?? 0, 'ar'),
    treasury: (data) => getTreasuryName(data),
    note: (data) => {
      const comments = Array.isArray(data?.sandE)
        ? data.sandE.map((entry) => entry?.sand_comment).filter(Boolean)
...
export async function mapDataToTemplateParams(docType, data, user = null) {
  const resolveMapper = (type, seen = new Set()) => {
    if (seen.has(type)) return {};
    seen.add(type);
    let map = PRINTABLE_PARAM_MAPPERS[String(type)] || {};
    if (typeof map === 'string') return resolveMapper(map, seen);
    if (!map.__inherit) return map;
    const { __inherit, ...childMap } = map;
    return { ...resolveMapper(__inherit, seen), ...childMap };
  };
  const map = resolveMapper(docType);

  const out = {};

  // Generic document support: expose entry columns plus JSON payload fields.
  // New document designs need only placeholders matching their configured field names.
  const payloads = [data, data?.data, data?.DataJson, data?.FormConfigSnapshotJson]
    .map((value) => {
  ...
  payloads.forEach((payload) => Object.entries(payload).forEach(([key, value]) => {
    if (value !== undefined && value !== null && typeof value !== 'object') out[key] = value;
  }));
  out.created_at = out.created_at || formatDate(data?.CreatedAt || data?.created_at || data?.UpdatedAt);
  out.DocumentEntryID = out.DocumentEntryID || data?.DocumentEntryID || data?.ID || '';
  out.EntityID = out.EntityID || data?.EntityID || '';
  // Document entries store user fields in DataJson; flatten them into template params.
  if (typeof data?.DataJson === 'string') {
    try {
      const fields = JSON.parse(data.DataJson);
      if (fields && typeof fields === 'object' && !Array.isArray(fields)) Object.assign(out, fields);
    } catch {}
  }
  ...
  if (!out.logo && logoUrl) out.logo = logoUrl;
  return out;
}
