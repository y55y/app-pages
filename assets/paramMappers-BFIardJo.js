...
export const PRINTABLE_PARAM_MAPPERS = {
  ...
  buy_inv: {
    ...
    supplier_name: (data) => {
      const d = data.data || data;
      return d.SupplierName || getClientOrVendorName(d);
    },
    supplier_nameE: (data) => {
      const d = data.data || data;
      return d.SupplierNameE || d.SupplierName || getClientOrVendorName(d);
    },
    supplier_address: (data) => {
      const d = data.data || data;
      return d.SupplierAddress || d.ClientAddress || '';
    },
    supplier_addressE: (data) => {
      const d = data.data || data;
      return d.SupplierAddressE || d.SupplierAddress || d.ClientAddressE || d.ClientAddress || '';
    },
    supplier_vat: (data) => {
      const d = data.data || data;
      return d.Supplier_vat || d.SupplierVAT || d.Client_vat || '';
    },
    supplier_crn: (data) => {
      const d = data.data || data;
      return d.Supplier_crn || d.SupplierCRN || d.Client_CRN || '';
    },
    ...
    subtotal: (data) => {
      const d = data.data || data;
      if (d.SubTotal || d.Subtotal) return d.SubTotal || d.Subtotal || 0;
    ...
    total_discount: (data) => {
      const d = data.data || data;
      if (d.dis || d.discount) return d.dis || d.discount || 0;
      const items = d.order_entries || d.Items || [];
      if (!Array.isArray(items)) return 0;
      return items.reduce((sum, item) => sum + Number(item.discount || item.dis || 0), 0);
    },
    total_tax: (data) => {
      const d = data.data || data;
      if (d.vat || d.VAT || d.Tax) return d.vat || d.VAT || d.Tax || 0;
      const items = d.order_entries || d.Items || [];
      if (!Array.isArray(items)) return 0;
      return items.reduce((sum, item) => {
        const qty = Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0);
