  ...
  sell_inv: {
    ...
    items: (data) => {
      ...
      return rawItems.map((item, index) => {
        ...
        const taxRate = Number(item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate ?? 15);
        
        const taxableAmount = Math.round((qty * rate - discount) * 100) / 100;
        const taxValue = Number(item.TaxA ?? Math.round((taxableAmount * taxRate / 100) * 100) / 100);
    ...
    total_tax: (data) => {
      ...
      const rowVat = entries.reduce((sum, item) => {
        ...
        const discount = Number(item.discount || item.dis || 0);
        const rate = Number(item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate ?? 15);
        return sum + Number(item.TaxA ?? ((qty * price - discount) * rate / 100));
      }, 0);
    ...
    total_amount: (data) => {
      const d = data.data || data;
      const entries = d.order_entries || d.Items || [];
      if (!entries.length) return formatAmount(d.Total || 0);
      const subtotal = entries.reduce((sum, item) => sum + Number(item.total ?? ((Number(item.QuantityOnOrder || item.Qty || item.Quantity || 0)) * Number(item.unitPrice || item.Rate || item.Price || 0))), 0);
      const vat = entries.reduce((sum, item) => {
        const rate = Number(item.TaxRateSnapshot ?? item.tax_rate ?? item.TaxRate ?? 15);
        return sum + Number(item.TaxA ?? ((Number(item.total || 0)) * rate / 100));
      }, 0);
      return formatAmount(subtotal + vat - Number(d.dis ?? d.discount ?? 0));
    },
