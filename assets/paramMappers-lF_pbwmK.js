...
export const PRINTABLE_PARAM_MAPPERS = {
  ...
  sell_inv: {
    ...
    total_tax: (data) => {
      const d = data.data || data;
      let vat = Number(d.vat ?? d.VAT ?? d.Tax ?? 0);
      if (vat === 0) {
        let sub = Number(d.SubTotal || d.Subtotal || 0);
        if (sub === 0 && d.order_entries) {
          sub = d.order_entries.reduce((sum, item) => {
        ...
        const disc = Number(d.dis ?? d.discount ?? 0);
        const taxRate = Number(d.tax_rate || d.TaxRate || 15);
        vat = (sub - disc) * taxRate / 100;
      }
      return formatAmount(vat);
    },
    ...
    salesman: (data) => { const d = data.data || data; return d.CreatorName || d.SalesmanName || d.SellerName || d.UserName || d.Salesman || d.salesman || ''; },
