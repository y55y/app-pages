const a=`// Central mapping from backend resource fields to HTML template params per doc type
...
export function mapDataToTemplateParams(docType, data, user = null) {
  ...
  Object.entries(map).forEach(([paramKey, rule]) => {
    try {
      ...
      // Override with company information if it's a site field
      if (paramKey.startsWith('site_') || paramKey === 'business_name' || paramKey === 'footer') {
